#!/usr/bin/env node
/**
 * Remove curated 80 palettes (生椰拿铁 … 安眠) from D1 / R2.
 * Usage:
 *   node scripts/delete-curated-seed.mjs --remote --r2 --clean-local
 */
import fs from 'fs';
...
const MANIFEST = path.join(ROOT, 'data', 'seed-palettes', 'curated-80', 'manifest.json');
const SQL_FILE = path.join(ROOT, 'data', 'seed-palettes', 'curated-80', 'delete-curated-80.sql');
...
function buildDeleteSql(entries) {
  ...
  return `-- Remove curated 80 palettes (${entries.length} rows)
...
function cleanLocalSeedArtifacts(entries) {
  const importSql = path.join(ROOT, 'data', 'seed-palettes', 'curated-80', 'import.sql');
  fs.writeFileSync(importSql, '-- curated-80 palettes removed.\n');
  fs.writeFileSync(MANIFEST, '[]\n');

  const imagesDir = path.join(ROOT, 'data', 'seed-palettes', 'curated-80', 'images');
...
function main() {
  ...
  if (entries.length === 0) {
    console.log('No curated palette IDs found.');
    return;
  }
  ...
  if (process.argv.includes('--clean-local')) {
    ...
    console.log('Local curated seed artifacts cleaned.');
  }
