#!/usr/bin/env node
/**
 * Remove v3-88 palettes from D1 / R2.
 * Usage:
 *   node scripts/delete-v3-seed.mjs --remote --r2 --clean-local
 */
import fs from 'fs';
...
const MANIFEST  = path.join(ROOT, 'data', 'seed-palettes', 'v3-88', 'manifest.json');
const SQL_FILE  = path.join(ROOT, 'data', 'seed-palettes', 'v3-88', 'delete-v3-88.sql');
...
function buildDeleteSql(entries) {
  ...
  return `-- Remove v3-88 palettes (${entries.length} rows)\nPRAGMA foreign_keys = ON;\nDELETE FROM styles\nWHERE user_id = '${SEED_USER_ID}'\n  AND id IN (\n  ${ids}\n);\n`;
}

function cleanLocalSeedArtifacts(entries) {
  const importSql = path.join(ROOT, 'data', 'seed-palettes', 'v3-88', 'import.sql');
  if (fs.existsSync(importSql)) fs.writeFileSync(importSql, '-- v3-88 palettes removed.\n');
  fs.writeFileSync(MANIFEST, '[]\n');
  const imagesDir = path.join(ROOT, 'data', 'seed-palettes', 'v3-88', 'images');
...
function deleteFromR2(entries) {
  ...
  for (const entry of entries) {
    ...
    const r = spawnSync('npx', ['wrangler', 'r2', 'object', 'delete', `${R2_BUCKET}/${key}`, '--remote'], { cwd: ROOT, stdio: 'inherit' });
    if (r.status !== 0) failed += 1;
  }
  if (failed > 0) { console.error(`R2 cleanup finished with ${failed} error(s).`); process.exit(1); }
  console.log('R2 cleanup done.');
}

function main() {
  const entries = loadEntries();
  if (entries.length === 0) { console.log('No v3 palette IDs found.'); return; }
  ...
  if (process.argv.includes('--remote') || process.argv.includes('--local')) {
    ...
    const d1 = spawnSync('npx', ['wrangler', 'd1', 'execute', 'genom-db', flag, `--file=${SQL_FILE}`], { cwd: ROOT, stdio: 'inherit' });
    if (d1.status !== 0) process.exit(d1.status ?? 1);
    console.log('D1 delete complete.');
  }
  if (process.argv.includes('--r2')) deleteFromR2(entries);
  if (process.argv.includes('--clean-local')) {
    cleanLocalSeedArtifacts(entries);
    console.log('Local v3 seed artifacts cleaned.');
  }
}
