...
/**
 * Color Walk 五色收藏页：固定 5 格竖列，空位虚线，满额提示。
 */
export default function ColorWalkSavedColorsPage({
  ...
  return (
    <div className="relative z-10 mx-auto flex w-full max-w-md min-h-0 flex-1 flex-col px-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] pt-1 md:px-8">
          ...
          {Array.from({ length: COLOR_WALK_SAVED_MAX }, (_, i) => {
            ...
            if (!item) {
              return (
                <div
                  key={`empty-${i}`}
                  className="flex min-h-[5.75rem] flex-col items-center justify-center rounded-2xl border border-dashed border-zen-ink/20 bg-white/40 px-4 py-5 text-center"
                >
            ...
            return (
              <div
                ...
                style={{ backgroundColor: item.hex, minHeight: '5.75rem' }}
              >
                <div className="absolute inset-0 bg-black/10" aria-hidden />
                <div className="relative flex min-h-[5.75rem] items-center justify-between gap-3 p-3.5">
                  <div className="min-w-0 flex-1" style={{ color: labelColor }}>
                    <p className="truncate font-zenSerif text-xl font-medium drop-shadow-sm">{name}</p>
