...
import { SEA_COLOR_HUE_TAGS } from './seaTagVocabulary';
...
function deriveMeaning(tags, overview, designLogic) {
  ...
  const domain = tags.find((t) => SEA_COLOR_HUE_TAGS.includes(t) || /色系$/.test(t));
  ...
  else if (tags.includes('低饱和')) bits.push('灰度克制，气质沉静');
...
function derivePsychology(tags) {
  ...
  if (tags.includes('低饱和')) lines.push('低饱和降低情绪振幅，利于长时间阅读与冥想式体验。');
...
function deriveUsage(tags) {
  ...
  if (tags.includes('低饱和')) uses.push('画册、建筑摄影与极简 UI');
  if (tags.includes('高饱和')) uses.push('活动主视觉、短视频封面与潮流周边');
  if (tags.includes('互补色')) uses.push('插画、潮玩与需要冲击力的主画面');
